...
import os
import numpy as np
from datasets import scene_util
...
class ScannetDataset(data.Dataset):
    def __init__(self, root, npoints=8192, split='train'):
        ...
        self.root = root
        self.split = split
        self.data_filename = os.path.join(self.root, f'scannet_{split}.pickle')
...
class ScannetDatasetWholeScene(data.Dataset):
    def __init__(self, root, npoints=8192, split='train'):
        ...
        self.split = split
        self.data_filename = os.path.join(self.root, f'scannet_{split}.pickle')
        print('Loading data from {}...'.format(self.data_filename))
        with open(self.data_filename, 'rb') as fp:
            self.scene_points_list = pickle.load(fp, encoding='latin1')
...
if __name__ == '__main__':
    # d = ScannetDatasetWholeScene(root='', split='test', npoints=8192)
    # labelweights_vox = np.zeros(21)
    # for ii in range(len(d)):
    #     print(ii)
    #     ps, seg, smpw = d[ii]
    #     for b in range(ps.shape[0]):
    #         print("seg shape:", seg[b, smpw[b, :] > 0].shape)
    #
    ...
    train_dataset = ScannetDataset(root='./scannet_scenes/data', split='train', npoints=8192)
    train_dataset_whole = ScannetDatasetWholeScene(root='./scannet_scenes/data', split='test', npoints=8192)
    # train_loader = DataLoader(ScannetDataset(root='./scannet_scenes/data',
    #                                          split='train', npoints=8192),
    #                           num_workers=16, batch_size=16, shuffle=False, drop_last=False)

    print("Dataset: train_dataset")
    for point_sets, semantic_segs, sample_weights in train_dataset:
        print(point_sets.shape)
        print(semantic_segs.shape)
        print(sample_weights.shape)
        break
    print("Dataset: train_dataset_whole")
    for point_sets, semantic_segs, sample_weights in train_dataset_whole:
        print(point_sets.shape)
        print(semantic_segs.shape)
        print(sample_weights.shape)
        break